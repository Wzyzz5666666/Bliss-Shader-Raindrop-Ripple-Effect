...
if (gl_FragCoord.x * texelSize.x < 1.0  && gl_FragCoord.y * texelSize.y < 1.0 )	{
	...
	// Keep a stable normal for diffuse lighting; the full animated normal is
	// retained below for refraction and specular reflection.
	vec3 diffuseNormal = normal;

	#if defined OVERWORLD_SHADER && defined RAIN_SPLASH_EFFECT && !defined HAND && !defined ENTITIES
		if (iswater > 0.95 && max(rainStrength, wetness) > 0.01) {
			float waterSkyExposure = BlissRainSkyExposure(lmtexcoord.w);
	...
	vec3 WS_normal = viewToWorld(diffuseNormal);
	//////////////////////////////// 
	//////////////////////////////// DIFFUSE LIGHTING
	//////////////////////////////// 
	...
	#ifdef OVERWORLD_SHADER
		float NdotL = clamp(dot(diffuseNormal, normalize(WsunVec*mat3(gbufferModelViewInverse))),0.0,1.0); NdotL = clamp((-15 + NdotL*255.0) / 240.0  ,0.0,1.0);
	...
	//////////////////////////////// 
		...
		#ifdef OVERWORLD_SHADER
			if (iswater > 0.9 && max(rainStrength, wetness) > 0.01) {
				float rippleExposure = max(rainStrength, wetness) * BlissRainSkyExposure(lmtexcoord.w);
				roughness = max(roughness, mix(0.05, 0.12, clamp(rippleExposure, 0.0, 1.0)));
			}
		#endif
		...
		if (iswater > 0.0 && gl_FragData[0].a < 0.9999999){
			...
			// Sun, Sky, and screen-space reflections
					...
					SunReflection = (lightCol.rgb / 80.0) * Shadows * GGX(normal, -normalize(viewPos), WsunVec*mat3(gbufferModelViewInverse), roughness, vec3(f0)) ; 
				#endif
				...
				if(isEyeInWater == 1 && iswater > 0.9){
					vec3 underwaterReflection = skyCloudsFromTex(mat3(gbufferModelViewInverse) * reflectedVector, colortex4).rgb / 30.0;
				 	SkyReflection.rgb = underwaterReflection * exp(-8.0 * vec3(Water_Absorb_R, Water_Absorb_G, Water_Absorb_B));
				}
			#endif
