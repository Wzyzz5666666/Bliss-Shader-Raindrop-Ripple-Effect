...
void main() {
	...
	if (swappedDepth >= 1.0) {
			...
			#else
				#if RESOURCEPACK_SKY == 1 || defined VANILLA_MOON
					Background += toLinear(texture2D(colortex10, texcoord).rgb * (255.0 * 2.0));
				#endif
		...
		// Rain ripple normals are deliberately kept for the specular path, where
		// the moving rings catch the sun/sky highlight.  Using the same high
		// frequency normal for Bliss' steep diffuse curve produces dark rings in
		// otherwise unlit areas, unlike interactionT's wet-surface response.
		float puddleWet = 0.0;
		vec3 diffuseNormal = slopednormal;
			...
			if (!iswater && !isPortal && !entities && !isLeaf && !hand && !isBeacon && !isGrass) {
				...
				puddleWet = BlissPuddleMask(puddleWorldPosition, puddleSkyExposure, max(wetness, rainStrength) * noPuddleAreas, puddleWorldNormal.y);
				float puddleCoverage = smoothstep(0.05, 0.45, puddleWet);
				float diffuseRippleWeight = mix(1.0, RIPPLE_DIFFUSE_STRENGTH, puddleCoverage);
				diffuseNormal = normalize(mix(puddleWorldNormal, slopednormal, diffuseRippleWeight));
			}
		#endif
	...
	////////////////////////////////////////////////////////////////////////////////////
		...
		NdotL = clamp((-15 + dot(diffuseNormal, WsunVec)*255.0) / 240.0  ,0.0,1.0);
	...
	/////////////////////////////////////////////////////////////////////////////////////
		...
		// GTAO
			...
			if (!hand && !isBeacon) AO = ambient_occlusion(vec3(texcoord/RENDER_SCALE-TAA_Offset*texelSize*0.5,z), viewPos, worldToView(slopednormal), r2) * vec3(1.0);
		...
		// RTAO and/or SSGI
			...
			if (!hand && !isBeacon) ApplySSRT(Indirect_lighting, viewPos, normal, vec3(bnoise, noise_2), lightmap.xy, AmbientLightColor*2.5, vec3(TORCH_R,TORCH_G,TORCH_B), isGrass);
		#endif
	...
	/////////////////////////////////////////////////////////////////////////
		...
		if (isBeacon) {
			// A beacon has no LabPBR alpha. Make its emissive output independent
			// of the selected indirect-light algorithm so GTAO cannot suppress it.
			gl_FragData[0].rgb = albedo * Emissive_Brightness;
		} else {
	...
	////// DEBUG VIEW STUFF
		...
		if(heightRelativeToClouds > 0.0 && !hand && !isBeacon){
