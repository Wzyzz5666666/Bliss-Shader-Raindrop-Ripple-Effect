...
void main() {
	...
	bool isBeacon = abs(dataUnpacked1.w-0.80) < 0.01;
	...
	gl_FragData[1] = vec4(0.0,0.0,0.0,texture2D(colortex14,texcoord).a);
		...
		// Beacons are emissive geometry, not a surface that should receive AO.
		gl_FragData[1].xy = SSAO(viewPos, FlatNormals, hand || isBeacon, isLeaf, noise);
	#endif
