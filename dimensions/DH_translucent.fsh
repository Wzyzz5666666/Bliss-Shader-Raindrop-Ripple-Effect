...
uniform sampler2D blissRainRippleTex1;
uniform sampler2D blissRainRippleTex2;
uniform sampler2D blissRainRippleTex3;
...
if (gl_FragCoord.x * texelSize.x < 1.0  && gl_FragCoord.y * texelSize.y < 1.0 )	{
    ...
    // Use the full animated normal for reflections, but keep diffuse lighting
    // mostly on the stable water/terrain normal to avoid dark ripple rings.
    vec3 diffuseNormals = normals;
    
    #if defined OVERWORLD_SHADER && defined RAIN_SPLASH_EFFECT
        if (iswater && max(rainStrength, wetness) > 0.01) {
            float waterSkyExposure = BlissRainSkyExposure(lightmapCoords.y);
            waterRippleWet = max(rainStrength, wetness) * waterSkyExposure;
            if (waterRippleWet > 0.001) {
                float rippleProfile = BlissPuddleNoise(playerPos + cameraPosition);
                vec3 rippleNormal = BlissRainRippleNormal(playerPos + cameraPosition, rippleProfile, blissRainRippleTex1, blissRainRippleTex2, blissRainRippleTex3, 0.5);
                float rippleInfluence = rippleProfile * waterRippleWet * saturate(normals.y * 0.5 + 0.5);
                normals = BlissApplyRippleNormal(normalize(normals), vec3(rippleNormal.x, 1.0, rippleNormal.y), rippleInfluence);
                diffuseNormals = normalize(mix(diffuseNormals, normals, RIPPLE_DIFFUSE_STRENGTH));
            }
        }
    #endif
    
    normals = worldToView(normals);
	diffuseNormals = worldToView(diffuseNormals);
    ...
    vec3 Specular_lighting = lightCol.rgb/80.0;

    float NdotL = max(dot(diffuseNormals, WsunVec2), 0.0f);
    Direct_lighting *= NdotL;
    
    #ifdef CLOUDS_SHADOWS
     vec3 cloudShadow = GetCloudShadow(playerPos);
     Direct_lighting *= cloudShadow;
     Specular_lighting *= cloudShadow;
    #endif
    ...
    fresnel = mix(0.02, 1.0, fresnel); 
        ...
        float waterRoughness = 0.05;
            ...
            waterRoughness = mix(0.05, 0.12, rainStrength * BlissRainSkyExposure(lightmapCoords.y));
        #endif
        vec3 sunReflection = Specular_lighting * GGX(normals, -normalize(pos.xyz), WsunVec2, waterRoughness, vec3(0.02)) * (1-ssReflections.a) ;
