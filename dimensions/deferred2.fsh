...
#ifdef VOXY
	// Voxy translucents are rendered before the vanilla translucent stages.
	// Prefer vanilla's buffer wherever vanilla depth exists; otherwise merge
	// the early Voxy result into Bliss' regular forward/translucent buffer.
	bool vanillaDepthExists = texelFetch(depthtex0, ivec2(gl_FragCoord.xy), 0).x < 1.0;
	gl_FragData[1] = vanillaDepthExists
		? texelFetch(colortex2, ivec2(gl_FragCoord.xy), 0)
		: texelFetch(colortex16, ivec2(gl_FragCoord.xy), 0);
#endif
}
