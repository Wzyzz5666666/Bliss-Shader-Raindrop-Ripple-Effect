...
void main() {
	...
	float lightmap = clamp((lmtexcoord.w - 0.8) * 10.0, 0.0, 1.0);
	float skyExposure = BlissRainSkyExposure(lmtexcoord.w);
	...
	//////////////////////////////// 				//////////////////////////////// 
		...
		normal = applyBump(tbnMatrix, NormalTex.xyz, 1.0 - Puddle_shape * Puddle_shape);
	#endif
		...
		if (groundRippleWet > 0.001) {
			...
			#ifdef MC_NORMAL_MAP
			normal = BlissApplyRippleTangentNormal(normal, tbnMatrix, rippleNormal, groundRippleWet);
			#else
			groundWorldNormal = BlissApplyRippleNormal(groundWorldNormal, vec3(rippleNormal.x, 1.0, rippleNormal.y), groundRippleWet);
			normal = normalize(worldToView(groundWorldNormal));
			#endif
		}
	#endif
	...
	//////////////////////////////// 				//////////////////////////////// 
		...
		SpecularTex.r = mix(SpecularTex.r, 1.0, clamp(Puddle_shape, 0.0, 1.0));
	...
	//////////////////////////////// 				////////////////////////////////
		...
		#ifdef Puddles
			float wetDarkening = clamp(Puddle_shape, 0.0, 0.2) / 0.2;
			if(SpecularTex.g < 229.5/255.0) Albedo.rgb = pow(max(Albedo.rgb, vec3(0.0)), vec3(mix(1.0, 1.15, wetDarkening)));
		#endif
